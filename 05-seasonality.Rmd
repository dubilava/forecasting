...
`r newthought('Seasonality characterizes')` some economic time series. Prices of agricultural commodities, for example, tend to have a seasonal pattern, owing to relative abundance of the commodity after harvest, and their gradual depletion as the year progresses. Energy sales also tend to have seasonal pattern, due to relatively high demand for energy during the winter (need for heating) and summer (need for cooling) months. Seasonality is a self-repeating pattern within a calendar year that arises from the links of economic outcomes to the calendar.
...
We typically model seasonality as a monthly or quarterly pattern, but it can also be modeled as a higher frequency pattern (e.g. weekly). 
...
Otherwise, and perhaps more interestingly, we can directly model seasonality in a regression setting by incorporating in it the seasonal component.
...
Consider a weekly series of initial claims filed by an unemployed individual after a separation from an employer in the United States (sourced from the online data portal of the Federal Reserve Bank of St. Louis) spanning the January 1993 -- December 2008 period. 
...
An apparent seasonal pattern is present in the series. We will approximate the seasonality using Fourier series. Specifically, we estimate the model parameters by regressing claims, which we denote by $y$, on sine and cosine terms of up to order two, from a Fourier series. Fitted values are then given by: $\hat{y}_t = \hat{\alpha}+\sum_{k=1}^{2}\left[\hat{\beta}_{1k}\sin\left(\frac{2\pi kt}{52.143}\right)+\hat{\beta}_{2k}\cos\left(\frac{2\pi kt}{52.143}\right)\right]$, which we overlay onto the time series:
...
The fitted series mimic the general seasonal pattern reasonably well. There are some major departures at times, however. Especially so in the first week of the year, for example. 
