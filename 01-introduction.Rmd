...
gg_ts <- ggplot(dt,aes(x=t,y=x))+
  ...
  geom_point(color="dimgray",fill="lightgray",stroke=1,shape=21,size=4)+
  ylim(0,7)+
  labs(x=expression(t),y=expression(x[t]))+
  theme_eg()

gg_hist <- ggplot(dt,aes(x=x))+
  geom_dotplot(binwidth=.3,color="dimgray",fill="lightgray",stroke=2,stackratio=1.1)+
  xlim(0,7)+
  coord_flip()+
  theme_eg()+
...
gg_loss <- ggplot()+
  ...
  geom_text(data=data.table(x=rep(0,2),y=seq(5,10,5)),aes(x=x,y=y),label=seq(5,10,5),hjust=1,nudge_x=-.4,size=4.5,color="dimgray",family="Segoe Print")+
  ...
  geom_point(data=dt,aes(x=e,y=0),color="dimgray",fill="lightgray",stroke=1,shape=21,size=3)+
  geom_point(data=dt,aes(x=0,y=l),color="dimgray",fill="coral",stroke=1,shape=21,size=3)+
  xlim(-3.5,3.5)+
  ylim(0,11)+
  coord_flip()+
  theme_eg()+
