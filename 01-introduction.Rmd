...
gg_ts <- ggplot(dt,aes(x=t,y=x))+
  ...
  geom_point(color="black",fill="gray",stroke=1,shape=21,size=4)+
  ylim(0,7)+
...
```{r loss-function, fig.cap="Forecast error and loss function", echo=FALSE, message=FALSE}
n <- 7
set.seed(4)
...
gg_loss <- ggplot()+
  ...
  geom_point(data=dt,aes(x=e,y=0),color="black",fill="gray",stroke=1,shape=21,size=3)+
  geom_point(data=dt,aes(x=0,y=l),color="black",fill="black",stroke=1,shape=21,size=3)+
  xlim(-3.5,3.5)+
  ylim(0,11)+
  coord_flip()+
  theme_eg()+
...
plot_grid(gg_errors,gg_loss,align="hv",ncol=2,rel_widths = c(2,2))
```
