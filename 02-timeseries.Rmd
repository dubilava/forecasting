...
Autocorrelations of the stationary and ergodic stochastic process dissipate to zero as $k$ increases. Thus, by observing autocorrelograms, in conjunction with a visual inspection of the time series, we can get an idea whether the time series are stationary.^[There are, of course, formal tests of stationarity, which we will discuss in one of the subsequent chapters.] If the time series are non-stationary, we can make changes to it, so that the transformed series are stationary.
