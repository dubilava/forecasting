...
\caption{\label{fig:acf}Autocorrelogram}
\end{figure}
...
\includegraphics{forecasting_files/figure-latex/ts-1.pdf} \includegraphics{forecasting_files/figure-latex/ts-2.pdf} \includegraphics{forecasting_files/figure-latex/ts-3.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# set the length of a time series to 180 (e.g., 15 years of monthly data)}
\NormalTok{n <-}\StringTok{ }\DecValTok{180}

\CommentTok{# set seed to 2 and generate a sequence of standard normal error terms}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{2}\NormalTok{)}
\NormalTok{e <-}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(n)}

\CommentTok{# suppose the data generating process of a time series is: }
\CommentTok{# y_\{t\} = 0.7*y_\{t-1\}+0.01*t+e_\{t\}}
\CommentTok{# simulate these series, assume y_\{0\} = 0.}
\NormalTok{y <-}\StringTok{ }\NormalTok{e}

\NormalTok{y[}\DecValTok{1}\NormalTok{] <-}\StringTok{ }\FloatTok{0.01}\OperatorTok{+}\NormalTok{e[}\DecValTok{1}\NormalTok{]}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{2}\OperatorTok{:}\NormalTok{n)\{}
\NormalTok{  y[i] <-}\StringTok{ }\FloatTok{0.7}\OperatorTok{*}\NormalTok{y[i}\DecValTok{-1}\NormalTok{]}\OperatorTok{+}\FloatTok{0.05}\OperatorTok{*}\NormalTok{i}\FloatTok{-0.0002}\OperatorTok{*}\NormalTok{i}\OperatorTok{^}\DecValTok{2}\OperatorTok{+}\NormalTok{e[i]}
\NormalTok{\}}

\CommentTok{# store the simulated series in a data.table, where the first column is a column of dates}
\NormalTok{dt <-}\StringTok{ }\KeywordTok{data.table}\NormalTok{(}\DataTypeTok{year=}\KeywordTok{seq}\NormalTok{(}\KeywordTok{as.Date}\NormalTok{(}\StringTok{"1980-01-01"}\NormalTok{),}\DataTypeTok{by=}\StringTok{"quarter"}\NormalTok{,}\DataTypeTok{along.with=}\NormalTok{y),}\DataTypeTok{y=}\NormalTok{y,}\DataTypeTok{trend=}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{n))}

\CommentTok{# plot the series (using ggplot)}
\KeywordTok{ggplot}\NormalTok{(dt,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{year,}\DataTypeTok{y=}\NormalTok{y))}\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{size=}\DecValTok{1}\NormalTok{)}\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_classic}\NormalTok{()}\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis.title=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{14}\NormalTok{),}\DataTypeTok{axis.text=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{12}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# suppose we think the linear trend model is appropriate; use the first 120 observations }
\CommentTok{# to estimate trend, and using this estimatem generate forecasts for the remaining series}
\NormalTok{R <-}\StringTok{ }\KeywordTok{round}\NormalTok{(.}\DecValTok{75}\OperatorTok{*}\NormalTok{n)}

\NormalTok{reg <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(y[}\DecValTok{1}\OperatorTok{:}\NormalTok{R]}\OperatorTok{~}\NormalTok{trend[}\DecValTok{1}\OperatorTok{:}\NormalTok{R],}\DataTypeTok{data=}\NormalTok{dt)}

\CommentTok{# store the vector of coefficients}
\NormalTok{theta <-}\StringTok{ }\NormalTok{reg}\OperatorTok{$}\NormalTok{coefficients}

\CommentTok{# calculate the residual standard deviation}
\NormalTok{sigma <-}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\KeywordTok{crossprod}\NormalTok{(reg}\OperatorTok{$}\NormalTok{residuals)}\OperatorTok{/}\NormalTok{(R}\DecValTok{-2}\NormalTok{))}

\CommentTok{# generated and include in the dataset the fitted values and 90% CI bounds}
\NormalTok{dt[,}\StringTok{`}\DataTypeTok{:=}\StringTok{`}\NormalTok{(}\DataTypeTok{yhat=}\NormalTok{(theta[}\DecValTok{1}\NormalTok{]}\OperatorTok{+}\NormalTok{theta[}\DecValTok{2}\NormalTok{]}\OperatorTok{*}\NormalTok{trend))]}
\NormalTok{dt[,}\StringTok{`}\DataTypeTok{:=}\StringTok{`}\NormalTok{(}\DataTypeTok{y.lo=}\NormalTok{(yhat}\FloatTok{-1.96}\OperatorTok{*}\KeywordTok{as.vector}\NormalTok{(sigma)),}\DataTypeTok{y.hi=}\NormalTok{(yhat}\FloatTok{+1.96}\OperatorTok{*}\KeywordTok{as.vector}\NormalTok{(sigma)))]}

\CommentTok{# set the fitted values of the in-sample range to NA}
\NormalTok{dt[trend }\OperatorTok{<=}\StringTok{ }\NormalTok{R,}\KeywordTok{c}\NormalTok{(}\StringTok{"yhat"}\NormalTok{,}\StringTok{"y.lo"}\NormalTok{,}\StringTok{"y.hi"}\NormalTok{)] <-}\StringTok{ }\OtherTok{NA}

\CommentTok{# re-arrange the dataset into the "long" format (helpful for plotting)}
\NormalTok{dt_long <-}\StringTok{ }\KeywordTok{melt}\NormalTok{(dt[,.(year,y,yhat,y.lo,y.hi)],}\DataTypeTok{id.vars=}\StringTok{"year"}\NormalTok{)}

\CommentTok{# plot the out-of-sample forecasts along with the observed time series}
\KeywordTok{ggplot}\NormalTok{(dt_long,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{year,}\DataTypeTok{y=}\NormalTok{value,}\DataTypeTok{color=}\NormalTok{variable,}\DataTypeTok{linetype=}\NormalTok{variable))}\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{na.rm=}\NormalTok{T,}\DataTypeTok{size=}\DecValTok{1}\NormalTok{)}\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{c}\NormalTok{(}\StringTok{"y"}\NormalTok{,}\StringTok{"yhat"}\NormalTok{,}\StringTok{"y.lo"}\NormalTok{,}\StringTok{"y.hi"}\NormalTok{),}\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\StringTok{"time series"}\NormalTok{,}\StringTok{"point forecast"}\NormalTok{,}\StringTok{"lower bound"}\NormalTok{,}\StringTok{"upper bound"}\NormalTok{),}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"darkgray"}\NormalTok{,}\StringTok{"goldenrod"}\NormalTok{,}\StringTok{"black"}\NormalTok{,}\StringTok{"black"}\NormalTok{))}\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_linetype_manual}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{c}\NormalTok{(}\StringTok{"y"}\NormalTok{,}\StringTok{"yhat"}\NormalTok{,}\StringTok{"y.lo"}\NormalTok{,}\StringTok{"y.hi"}\NormalTok{),}\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\StringTok{"time series"}\NormalTok{,}\StringTok{"point forecast"}\NormalTok{,}\StringTok{"lower bound"}\NormalTok{,}\StringTok{"upper bound"}\NormalTok{),}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{))}\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_classic}\NormalTok{()}\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis.title=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{14}\NormalTok{),}\DataTypeTok{axis.text=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{12}\NormalTok{),}\DataTypeTok{legend.position =} \KeywordTok{c}\NormalTok{(.}\DecValTok{1}\NormalTok{,.}\DecValTok{9}\NormalTok{),}\DataTypeTok{legend.title=}\KeywordTok{element_blank}\NormalTok{())}
\end{Highlighting}
\end{Shaded}

\includegraphics{forecasting_files/figure-latex/unnamed-chunk-8-2.pdf}
...
Consider the first-order autoregression: \[y_t = \alpha + \beta_1 y_{t-1} + \varepsilon_t,\] where \(\alpha\) is a constant term; \(\beta_1\) is the \emph{persistence} parameter; and \(\varepsilon_t\) is a white noise process.

Note that a necessary and sufficient condition for an \(AR(1)\) process to be covariance stationary is that \(|\beta_1| < 1\). We can see this by substituting recursively the lagged equations into the lagged dependent variables:
\[
\begin{aligned}
...
And its \emph{unconditional variance} is: \[\gamma_0 = Var\left(y_t\right) = Var\left(\frac{\alpha}{1-\beta_1} + \sum_{i=0}^{\infty}\beta_1^i\varepsilon_{t-i}\right) = \frac{\sigma_{\varepsilon}^2}{1-\beta_1^2}\]

\emph{Autocovariance} is simply the covariance between \(y_t\) and \(y_{t-k}\), that is: \[\gamma_k = Cov(y_t,y_{t-k}) = E[(y_t - \mu)(y_{t-k} - \mu)] = E(y_t y_{t-k}) - \mu^2\]
...
In fact, autocorrelation of any order can be given as the autoregression parameter (which in this instance is equivalent to the persistence measure) to that power:
\[
\begin{aligned}
...
The autocorrelation of AR(1), given that \(|\beta_1| < 1\), is a geometric decay; the smaller is \(|\beta_1|\) the more rapid is the decay.
...
\includegraphics{forecasting_files/figure-latex/unnamed-chunk-27-1.pdf}
