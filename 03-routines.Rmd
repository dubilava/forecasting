...
Let $\hat{y}_{t+h|t}=E(y_{t+h}|\Omega_{t};\hat{\theta})$ be a point forecast^[We use 'hat' to emphasize that the forecast is based on parameter estimates rather than true parameters of the model. The subscript $t+h|t$ is to indicate that the forecast is made in period $t$.] for period $t+h$ made in period $t$, that is, using using the available information at the time, $\Omega_t$. This is our 'best guess' about the actual realization of the random variable in period $t+h$, $y_{t+h}$. The difference between the realization and the forecast of the random variable is the forecast error. That is, $$e_{t+h|t} = y_{t+h} - \hat{y}_{t+h|t}$$
...
$$\begin{aligned}
		e_{t+h} &= y_{t+h}-\hat{y}_{t+h|t}\;~~\text{(forecast uncertainty)}   \\
...
To the extent that uncertainty cannot be avoided, a forecaster is bound to commit forecast errors. The goal of the forecaster is to minimize the 'cost' associated with the forecast errors. This is achieved by minimizing the expected loss function.


## Loss Function

A loss function, $L(e_{t+h|t})$, can take many different forms so long as it satisfies the following properties:
$$\begin{aligned}
