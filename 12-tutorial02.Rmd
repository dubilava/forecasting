# Tutorial 2: Data Management and Plotting{-}
...
In this tutorial, we will introduce several simple R functions, and will perform a basic forecasting exercise. For this tutorial we will only need to load the `data.table` and `ggplot2` libraries, for data management and plotting, respectively.
...
It is customary to illustrate a time series via a line plot. In what follows, we add a date column to our dataset and then plot `y` in the chronological order:
```{r echo=TRUE}
